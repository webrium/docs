# JWT
The `Jwt` class issues and verifies signed JSON Web Tokens. It is the standard way to implement stateless authentication: a token is signed when a user logs in, sent back on every request, and verified on the server without needing to store session state.

A token carries a **payload** of claims (such as the user's ID), a **header** describing how it was signed, and a **signature** that proves the payload has not been altered. `Jwt` signs tokens with HMAC-SHA and verifies both the signature and the standard time-based claims for you.
