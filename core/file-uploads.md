# File Uploads
The `Upload` class is a secure file-upload handler with a fluent API. It reads files from `$_FILES`, validates them against size, extension, and MIME-type rules, and moves them safely to a destination directory.
