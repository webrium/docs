...
Routes are defined using the static `Route` class. You can register them anywhere — your application's entry point, a dedicated route file you `require`, or any file loaded during bootstrap.
...
> **In the full framework:** Route files live in `app/Routes/` and are loaded with `Route::source(['Web.php'])` from `public/index.php`. `Route::source()` is a convenience for resolving filenames against the `routes` directory and `require`-ing them — it is not required for routing itself to work.
